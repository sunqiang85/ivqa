# process the train answer and val answer to bow vector according to vocab.json

import config
import json
import numpy as np
from process.process_vocab_vqaeval import process_punctuation
...
class AnswerEncoder:
    ...
    def __init__(self, vocab_path, ann_path):
...
def econde_answer(input_path, out_path):
    answer_encoder = AnswerEncoder(config.vocabulary_path, input_path)
...
def main():
    # train
    out_path = config.txt_dir + "/train_bow_answer_feature.h5"
    econde_answer(config.train_ann_file, out_path)

    # val
    out_path = config.txt_dir + "/val_bow_answer_feature.h5"
